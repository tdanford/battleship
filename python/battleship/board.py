...
class PlacedShip: 
    ...
    def __init__(self, ship: Ship, board: 'Board', start: str, orientation: Orientation): 
        self.ship = ship 
...
def parse_coord(coord: str) -> Tuple[int, int]: 
    ...
    if not m: raise ValueError(f"Can't parse {coord} as a coordinate")
...
def coord_range(width: int, start_coord: str, orientation: Orientation) -> List[Tuple[int, int]]: 
    (start_row, start_col) = parse_coord(start_coord) 
    if orientation == Orientation.HORIZONTAL: 
        return [
            (start_row, start_col + i) for i in range(width)
        ]
    else: 
        return [
            (start_row + i, start_col) for i in range(width)
        ]
...
class Board: 
    ...
    def vertical_room(self, coord: str) -> int: 
        return self.find_space(coord, Direction.UP) + self.find_space(coord, Direction.DOWN) 

    def horizontal_room(self, coord: str) -> int: 
        return self.find_space(coord, Direction.LEFT) + self.find_space(coord, Direction.RIGHT)
    
    def list_space_coords(self, coord: str, dir: Direction, max_width: int) -> List[str]: 
        ...
        while r >= 0 and c >= 0 and r < ROWS and c < COLS and len(spaces) < max_width and not self.spots[r][c].is_shot: 
    ...
    def find_space(self, coord: str, dir: Direction) -> int: 
        ...
        # space < 4 because 4 + 1 is the maximum ship length, and we never care about 
        # space that a ship occupies that _isn't_ in this space.
        while r >= 0 and c >= 0 and r < ROWS and c < COLS and space < 4 and not self.spots[r][c].is_shot: 
            r += rdelta 
            c += cdelta 
            space += 1
        return space 
    ...
    def search_spots(self, predicate: Callable[[Spot], bool]) -> Generator[Spot, None, None]: 
        for i in range(ROWS): 
            for j in range(COLS): 

...
class RandomComputerPlayer(Player): 
    ...
    def setup(self):
        ...
        self.deliver_message(Message(source=self.name, type="setup_complete", payload={}))
...
class HuntingComputerPlayer(Player): 
    ...
    def choose_shot(self):
        if self.state == HuntingState.SEARCHING: 
            spots = [format_coord(s.row, s.col) for s in self.target_board.unshot_spots()] 
    ...
    def shot_outcome(self, coord, outcome, sunk_ship):
        ...
        if self.state == HuntingState.SEARCHING: 
            if outcome == "hit": 
                ...
                self.logger.log(logging.INFO, f"A {outcome} switchees us to {self.state}")
                self.pending_directions = [dir for dir in Direction] 
                max_width = max([s.size() for (s, v) in self.remaining_ships.items() if v])
                self.pending = {
            ...
            if outcome == "miss": 
                ...
                # in that direction.
                self.logger.log(logging.INFO, f"A miss while HUNTING clears the {self.pending_directions[0]} direction")
        ...
        if sunk_ship is not None: 
            ...
            self.state = HuntingState.SEARCHING
            self.logger.log(logging.INFO, f"A sunk ship switches us back to {self.state}")
            self.pending = {}

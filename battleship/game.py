...
class Game(MessageTarget): 
    ...
    def __init__(self, name1: str, player1: MessageTarget, name2: str, player2: MessageTarget):
        self.logger = logging.getLogger("game")
        self.state = GameState.SETUP
        self.turn = 1
        self.names = (name1, name2)
        self.states = { 
            name1: PlayerState(name1), 
            name2: PlayerState(name2)
        }
        self.players = {
            name1: player1, 
            name2: player2
    ...
    def deliver_message(self, message):
        if message.type == "setup": 
            ...
            if self.state != GameState.SETUP: 
                raise ValueError(f"Illegal state {self.state} for {message.type} message")
            ...
            board.place_ship(
                PlacedShip(ship, board, start_coord, orientation)
            )
            ...
            if self.states[target_player].is_alive(): 
                ...
                self.logger.log(logging.INFO, f"Game ended with a win for {message.source}")
